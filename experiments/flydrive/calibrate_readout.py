...
sn=np.unique(np.r_[a[np.isin(a,sensg)|np.isin(b,sensg)],b[np.isin(a,sensg)|np.isin(b,sensg)]]);ban=set(sensg.tolist()+sn.tolist());rc=[int(x) for x in sel[np.argsort(wd[sel])[::-1]] if int(x) not in ban];read=loc[np.array(rc[:512])]
