...
SEED=int(sys.argv[1]);WIN=20;G=load(4000,SEED,112,256);tr=expert_episodes(SEED,60);va=expert_episodes(SEED+100,20);XO=np.concatenate([x for x,_ in tr]);yo=np.concatenate([y for _,y in tr]);XOV=np.concatenate([x for x,_ in va]);yov=np.concatenate([y for _,y in va]);ALL=[]
def feats(name,eps,seed):
 X=[];Y=[];wall=0.;events=0
 for j,(oo,aa) in enumerate(eps):
  s=SNN(G,name,seed+j);ev=s.events
  for o,a in zip(oo,aa):f,t=s.window(encode(o,G['input_n']),WIN);X.append(f);Y.append(a);wall+=t
  events+=s.events-ev
