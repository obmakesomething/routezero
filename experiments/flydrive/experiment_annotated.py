...
for SEED in [7,17,27]:
 ...
 for name in ['real','weight_shuffle','topology_shuffle','zero']:
  X,y,wall,events=feats(G,name,tr,SEED);XV,yv,_,_=feats(G,name,va,SEED+500);m=make_pipeline(StandardScaler(),LogisticRegression(max_iter=250,class_weight='balanced',solver='liblinear'));t=time.perf_counter();m.fit(X,y);fit=time.perf_counter()-t
 ...
 for name,m in [('linear',make_pipeline(StandardScaler(),LogisticRegression(max_iter=250,class_weight='balanced',solver='liblinear'))),('mlp',make_pipeline(StandardScaler(),MLPClassifier(hidden_layer_sizes=(32,32),max_iter=400,random_state=SEED)))]:
